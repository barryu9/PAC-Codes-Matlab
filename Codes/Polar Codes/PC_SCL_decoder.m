function d_esti = pc_SCL_decoder(pc_params,rp, llr_batch, L)
...
for batch_index=1:batchsize
    ...
    for phi = 0:N - 1
        ...
        if frozen_bits_mask(phi + 1) == 0 %if now we decode an unfrozen bit
            ...
            for l_index = 1:L
                ...
                t = (0~=0.5*(1-sign(P(1, l_index))));

                PM_pair(1, l_index) = PM(l_index)+abs(P(1, l_index))* t;
                PM_pair(2, l_index) = PM(l_index)+abs(P(1, l_index))* (1-t);
            end
            ...
            for l_index = 1:L
                ...
                PM(l_index) = PM(l_index)+abs(P(1, l_index))* (0~=0.5*(1-sign(P(1, l_index))));
                phi_mod_2 = mod(phi, 2);
                C(1, 2*l_index-1+phi_mod_2) = 0;
    ...
    if (crc_length > 0)
        for l_index = 1:length(PM_active)
            ...
            if err == 0
                d_esti(:,batch_index) = d_active(1:end-crc_length, path_num);
                break;
            else
